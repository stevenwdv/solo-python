...
def make_credential(serial, host, user, udp, prompt, pin,
    ...
    if sign_alg and sign_alg != "es256":
        ...
        signify_pk = base64.b64encode(b"Ed" + key_id + pk_bytes)
        if not no_pubkey:
    ...
    if key_file is not None:
        if sign_alg and sign_alg != "es256":
            ...
            print("Writing key file is only supported for signify/Minisign keys")
...
@click.option("--trusted-comment", default=None,
              help="Trusted comment included in global signature (with Minisign) "
...
@click.option("--untrusted-comment", default="signature created on solokey", show_default=True,
              help="Untrusted comment not included in global signature (with Minisign/EdDSA and --sig-file)")
@click.option("--key-id", default=None,
              help="Key ID to write to signature file (8 bytes as HEX) (with Minisign/EdDSA and --sig-file) "
                   "[default: <hash of credential ID>]")
@click.argument("credential-id")
@click.argument("filename")
def sign_file(pin, serial, udp, prompt, credential_id, host, filename, sig_file,
              alg, trusted_comment, untrusted_comment, key_id):
    ...
    def save_sig(sig_file_path, sig_format_descr, alg_id, untrusted_comment_bytes, key_id, file_signature,
                  trusted_comment_bytes=None, global_signature=None):
        with open(sig_file_path, "wb") as file:
            file.write(b"untrusted comment: ")
            file.write(untrusted_comment_bytes)
            file.write(b"\n")
            file.write(base64.b64encode(alg_id + key_id + file_signature))
            if global_signature:
                file.write(b"\ntrusted comment: ")
                file.write(trusted_comment_bytes)
                file.write(b"\n")
                file.write(base64.b64encode(global_signature))
            file.write(b"\n")

        print(f"Saved {sig_format_descr} signature using key {key_id_hex} to {sig_file_path}")

    def try_sign(sign_fun, cred_type, expected_host_prefix):
        try:
            return sign_fun()
    ...
    if alg == "eddsa":
        ...
        if alg == "minisign":
            ...
            ret = try_sign(lambda: dev.sign_hash(credential_id, dgst.digest(), pin, host, trusted_comment_bytes),
                           "EdDSA", host_prefix)
            ...
            ret = try_sign(lambda: dev.sign_hash(credential_id, dgst.digest(), pin, host), "ES256", host_prefix)
            file_signature = ret[1]
